...
export default async function TeachersPage() {
  ...
  return (
    <>
      <PageHeader
        title="Öğretmenler"
        description={isAdmin ? "Öğretmenler ve yöneticiler. Yönetici tüm dersleri görür." : "Her öğretmen kendi öğrencilerini ve derslerini yönetir."}
        actions={
          <>
          ...
          <TableBody>
            {teachers.map((t) => (
              <TableRow key={t.id} className={!t.active ? "opacity-60" : undefined}>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0 leading-tight">
                      <div className="flex items-center gap-2 truncate font-medium">
                        {t.name}
                        {t.role === "ADMIN" && <Badge className="bg-primary/10 text-primary">Yönetici</Badge>}
                        {t.id === me.id && <span className="text-xs font-normal text-muted-foreground">(siz)</span>}
                      </div>
                ...
                <TableCell>{t.id !== me.id && <TeacherRowActions teacher={{ id: t.id, name: t.name, active: t.active, role: t.role }} canManageRole={isAdmin} />}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
