...
export default async function LessonsPage({ searchParams }: PageProps<"/lessons">) {
  ...
  const scope = isTeacher ? { teacherId: user.id } : { students: { some: { studentId: user.id } } };
  ...
  return (
    <>
      ...
      <PageHeader
        title="Dersler"
        description={isTeacher ? "Planlanan, tamamlanan ve iptal edilen tüm dersler." : "Kayıtlı olduğunuz dersler."}
        actions={
          isTeacher && (
      ...
      {lessons.length === 0 ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {lessons.map((l) => <LessonCard key={l.id} lesson={l} isTeacher={isTeacher} />)}
        </div>
      )}
    </>
  );
